...
displayStatInfo(const struct stat *sb)
{
    ...
    printf("Last file access:         %ldns %s", sb->st_atim.tv_nsec, ctime(&sb->st_atime));
    printf("Last file modification:   %ldns %s", sb->st_mtim.tv_nsec, ctime(&sb->st_mtime));
    printf("Last status change:       %ldns %s", sb->st_ctim.tv_nsec, ctime(&sb->st_ctime));
}
